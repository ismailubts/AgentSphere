...
{#if !$mobile && !$showSidebar}
	<div
		class="as-sidebar-rail pt-[7px] pb-2 px-2 flex flex-col justify-between text-black dark:text-white h-full z-10 transition-all"
		id="sidebar"
	>
		<button
...
{#if $showSidebar}
	<div
		...
		class="h-screen max-h-[100dvh] min-h-screen select-none {$showSidebar
			? `${$mobile ? 'bg-gray-50 dark:bg-gray-950 as-sidebar-panel' : 'as-sidebar-panel'} z-50`
			...
			>
				...
				<a href="/" class="flex flex-1 px-0.5" on:click={newChatHandler}>
					<div
						...
						class="self-center as-sidebar-brand font-primary text-base"
					>
						{$WEBUI_NAME}
					</div>
				</a>
			...
			>
				<div class="pb-1.5">
					<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
						<a
							id="sidebar-new-chat-button"
							class="as-new-chat-btn group grow flex items-center space-x-3 rounded-2xl px-2.5 py-2 transition outline-none"
							href="/"
							draggable="false"
							on:click={newChatHandler}
							aria-label={$i18n.t('New Chat')}
						>
							...
							<div class="flex flex-1 self-center translate-y-[0.5px]">
								<div class="self-center text-sm font-primary text-white">{$i18n.t('New Chat')}</div>
							</div>
