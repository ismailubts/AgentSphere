...
<nav
	class="as-navbar sticky top-0 z-30 w-full {chat?.id
		? 'pt-0.5 pb-1'
...
>
	<div class="flex items-center w-full pl-1.5 pr-1">
		<div
			...
			class="{chat?.id
				? 'visible'
				: 'invisible'} as-navbar-gradient pointer-events-none absolute inset-0 -bottom-10 z-[-1]"
		></div>
				...
				{#if $mobile && !$showSidebar}
					...
					>
						<Tooltip content={$showSidebar ? $i18n.t('Close Sidebar') : $i18n.t('Open Sidebar')}>
							<button
								class="as-icon-btn cursor-pointer flex rounded-lg"
								on:click={() => {
				...
				<div class="self-start flex flex-none items-center text-gray-600 dark:text-gray-400">
					...
					{#if $user?.role === 'user' ? ($user?.permissions?.chat?.temporary ?? true) && !($user?.permissions?.chat?.temporary_enforced ?? false) : true}
						{#if !chat?.id}
							<Tooltip content={$i18n.t(`Temporary Chat`)}>
								<button
									class="as-icon-btn flex cursor-pointer px-2 py-2"
					...
					{#if $mobile && !$temporaryChatEnabled && chat && chat.id}
						<Tooltip content={$i18n.t('New Chat')}>
							<button
								class="as-icon-btn flex {$showSidebar
									? 'md:hidden'
									: ''} cursor-pointer px-2 py-2 text-gray-600 dark:text-gray-400"
								on:click={() => {
									initNewChat();
								}}
								aria-label="New Chat"
