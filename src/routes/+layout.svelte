<script>
		...
		if (type === 'calendar:alert' && data) {
			...
			if ($isLastActiveTab) {
				if ($settings?.notificationEnabled ?? false) {
					new Notification(`${data.title} • AgentSphere`, {
						body: timeStr,
		...
		if ((event.chat_id !== $chatId && !$temporaryChatEnabled) || isInBackground) {
			if (type === 'chat:completion') {
				...
				if (done) {
					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${displayTitle} • AgentSphere`, {
								body: contentPreview,
								icon: `${WEBUI_BASE_URL}/static/favicon.png`
							});
						}
	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isInBackground) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				...
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${title} • AgentSphere`, {
							body: data?.content,
	...
	const windowMessageEventHandler = async (event) => {
		if (
			!['https://github.com/ismailubts/AgentSphere', 'https://github.com/ismailubts/AgentSphere', 'http://localhost:9999'].includes(
				event.origin
			)
		) {
			return;
	...
	onMount(async () => {
		...
		if (backendConfig) {
			...
			const backendName = backendConfig.name?.trim();
			await WEBUI_NAME.set(
				backendName && !/open\s*web\s*ui/i.test(backendName) ? backendName : APP_NAME
			);
