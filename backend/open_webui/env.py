...
# BASE_DIR is the parent of BACKEND_DIR (agentsphere-dev/)
...
if FROM_INIT_PY:
    PACKAGE_DATA = {'version': importlib.metadata.version('agentsphere')}
else:
    try:
...
REDIS_KEY_PREFIX = os.getenv('REDIS_KEY_PREFIX', 'agentsphere')
...
# but is scoped to AgentSphere to avoid interfering with system-level settings.
...
def _parse_ssl_env(value: str) -> 'bool | _ssl.SSLContext':
    ...
    This allows users with corporate or internal CAs to point AgentSphere
...
if WEBUI_AUTH and WEBUI_SECRET_KEY == '':
    raise SystemExit(
        ...
        'start_windows.bat (Windows), or `agentsphere serve`.\n'
        ...
        'See https://github.com/ismailubts/AgentSphere'
    )
...
# Useful when AgentSphere sits behind a reverse proxy / API gateway that
# already uses the Authorization header for its own authentication — set
# this to a unique header (e.g. 'X-AgentSphere-Key') so the middleware
...
# compatible APIs for endpoints not natively handled by AgentSphere.
...
# Allows external apps to exchange OAuth tokens for AgentSphere tokens
...
WEBUI_NAME = os.getenv('WEBUI_NAME', 'AgentSphere')
if WEBUI_NAME != 'AgentSphere':
    WEBUI_NAME += ' (AgentSphere)'
...
FORWARD_USER_INFO_HEADER_USER_NAME = os.getenv('FORWARD_USER_INFO_HEADER_USER_NAME', 'X-AgentSphere-User-Name')
FORWARD_USER_INFO_HEADER_USER_ID = os.getenv('FORWARD_USER_INFO_HEADER_USER_ID', 'X-AgentSphere-User-Id')
FORWARD_USER_INFO_HEADER_USER_EMAIL = os.getenv('FORWARD_USER_INFO_HEADER_USER_EMAIL', 'X-AgentSphere-User-Email')
FORWARD_USER_INFO_HEADER_USER_ROLE = os.getenv('FORWARD_USER_INFO_HEADER_USER_ROLE', 'X-AgentSphere-User-Role')
FORWARD_SESSION_INFO_HEADER_MESSAGE_ID = os.getenv('FORWARD_SESSION_INFO_HEADER_MESSAGE_ID', 'X-AgentSphere-Message-Id')
FORWARD_SESSION_INFO_HEADER_CHAT_ID = os.getenv('FORWARD_SESSION_INFO_HEADER_CHAT_ID', 'X-AgentSphere-Chat-Id')

# If set while ENABLE_FORWARD_USER_INFO_HEADERS is True, send one signed HS256 JWT
# (FORWARD_USER_INFO_HEADER_JWT) instead of separate X-AgentSphere-User-* headers.
FORWARD_USER_INFO_HEADER_JWT_SECRET = (os.environ.get('FORWARD_USER_INFO_HEADER_JWT_SECRET') or '').strip() or None
FORWARD_USER_INFO_HEADER_JWT = os.environ.get('FORWARD_USER_INFO_HEADER_JWT', 'X-AgentSphere-User-Jwt')
try:
...
OTEL_SERVICE_NAME = os.getenv('OTEL_SERVICE_NAME', 'agentsphere')
